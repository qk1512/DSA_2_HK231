...
private:
	...
	void ReHeap_down(int index)
	{
		...
		if(leftChild < areaTable.size()){
			if (areaTable[leftChild]->size() < areaTable[largest]->size() ) largest = leftChild;
			else if (areaTable[leftChild] -> size () == areaTable[largest] -> size()){
				if(getIndex(areaTable[leftChild]) > getIndex(areaTable[largest])) largest = leftChild;
			}
		}

		if(rightChild < areaTable.size()){
			if (areaTable[rightChild]->size() < areaTable[largest]->size()) largest = rightChild;
			else if (areaTable[rightChild]->size() == areaTable[largest]->size()){
				if(getIndex(areaTable[rightChild]) > getIndex(areaTable[largest])) largest = rightChild;
			}
		}
